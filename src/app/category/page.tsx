...
//         className={`rounded-2xl w-full h-auto inline-block transition-opacity duration-500 ${
...
//               <button
//                 key={subSub.name}
//                 className={`px-4 py-1.5 rounded-full whitespace-nowrap min-w-28 text-xs sm:text-sm border transition ${
...
const categoryFromQuery = searchParams.get("category");
  ...
  useEffect(() => {
    fetchFolderTree().then((res) => {
      ...
      const foundCategory = res.data.find((c : any) => c.name === defaultCategory);
      const firstSub = foundCategory?.children?.find(
        (ch : any) => ch.type === "folder"
